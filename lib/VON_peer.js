...
function VONPeer(l_aoi_buffer, l_aoi_use_strict) {
    ...
    var log = LOG.newLayer('VON', 'VON', 'logs');
    ...
    var _forwardOverlapping = function(areaPacket, callback){
        ...
        var neighbour_j, id_j;

        var checklist; // array of neighbours that we have in common with the neighbour_i
        
        var rootDist;  // distance between j and the root

        var nearest; //Am I the nearest to the root?
        ...
        for (var key_i in _neighbours){
            ...
            // skip myself and any neighbours that have already recieved the message
            ...
            if (aoi.isPolygon) {
                if(_voro.collidesID_Polygon(id_i, aoi.points)){
                newNeighbours[key_i] = neighbour_i;
            }
            } else {
                if(_voro.collidesID(id_i, aoi.center, aoi.radius)){ // TODO: This does not work for Polygonal AOIs
                    newNeighbours[key_i] = neighbour_i;
                }
            }
        }

        // position of the root of the message and my distance to it
        ...
        for (var key_i in newNeighbours){
            ...
            // for each common neigbour of neighbour_i, see if they are closer to the root 
            for (var j = 0; j < checklist.length; j++){

                neighbour_j = _neighbours[checklist[j]];
                id_j = neighbour_j.id;

                // skip self or any new neighbours
                if (id_j === _self.id || Object.keys(newNeighbours).includes(checklist[j])){
                    continue;
                }

                rootDist = rootPos.distance(neighbour_j.aoi.center);

                // if the neighbour j is closer to the root than I am, 
                // I should not forward this message and can break the loop
                if (rootDist < myRootDistance){
                    nearest = false;
                    break;
                }
                if (rootDist == myRootDistance && id_j < _self.id){
                    nearest = false;
                    break;
                }            
            }
        ...
        switch (pack.type) {
            ...
            case VON_Message.MATCHER_AREA: {
                ...
                if ((closest === _self.id) && recipients.includes(_self.id)){
                    //log.warn('Area Message received more than once by center ['+_self.id+']');
                }
