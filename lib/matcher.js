...
;
            ...
            case Matcher_Message.MOVE_CLIENT : {
                ...
                if (pack.sender !== _id) {
                   // log.debug('Matcher['+_id+']: MOVE_CLIENT message');
            ...
            case Matcher_Message.MOVE_CLIENT_R :{
                //log.debug('Matcher['+_id+']: MOVE_CLIENT_R message');
	...
	var _initListeners = function(){
      ...
      // TODO: msg has a followClient field that should allow subscriptions to follow client, not implemented?
			 ...
			 log.debug('Matcher['+_id+']: Received subscribe message from client['+clientID+'] with subscription ID ['+msg.subID+']');
				_subscribe(clientID, msg.x, msg.y, msg.radius, msg.channel, msg.subID);
			});
    ...
    var _moveClient = this.move = function(clientID, x, y, packet){
        ...
        // update position of client object // check to see if clientID is on _clientList
  ...
  var _sendPublication = this.sendPublication = function(publication, allowForwarding) {
	...
	// Called when client requests a new subscription.
  // TODO - this should be provided by the client. Creates new sub object with unique ID
	var _subscribe = this.subscribe = function(clientID, x, y, radius, channel, subID) {
	...
	var mqttAuthenticate = this.mqttAuthenticate = function(client, username, password, callback) {
	  // Handle cases where password is undefined or not valid JSON
	  if (!password) {
	    // Allow connection without spatial coordinates (default position)
	    callback(null, true);
	    return;
	  }
	  ...
	  try {
	    ...
	    // If JSON parsing fails, allow connection without spatial coordinates
	    callback(null, true);
	  }
  }
