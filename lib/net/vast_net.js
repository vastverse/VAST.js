...
function encode(packet) {
    ...
    try {
      var encodedData = msgpack.encode(packet);

      const lengthPrefix = Buffer.alloc(4);
      lengthPrefix.writeUInt32BE(encodedData.length, 0); // Write the length of the encoded packet
      
      const encoded = Buffer.concat([lengthPrefix, encodedData]);
      return encoded;
    } catch (err) {
...
function vast_net(_localIP, onReceive, onConnect, onDisconnect, id) {
	...
	var _processData = function (socket, data) {
        ...
        if (!socket.recv_buf)
            socket.recv_buf = Buffer.alloc(0);

        // store data to buffer directly first
        // socket.recv_buf += data;
        socket.recv_buf = Buffer.concat([socket.recv_buf, data]);

        // console.log('recv_buf:', socket.recv_buf);

        var pack; // decoded message
        while (socket.recv_buf.length >= 4) {  // Check if we have at least 4 bytes for the length prefix
            const messageLength = socket.recv_buf.readUInt32BE(0);
    
            if (socket.recv_buf.length >= messageLength + 4) {  // Check if we have the full message
                const message = socket.recv_buf.slice(4, messageLength + 4);  // Include the length prefix
                socket.recv_buf = socket.recv_buf.slice(messageLength + 4);  // Remove the processed message from the buffer
                ...
                break;  // Wait for more data if the full message is not yet received
            }
