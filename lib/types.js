...
var l_area = exports.area = function (center, radius, points) {
    ...
    this.intersectsArea = function (area){
        if (this.points !== undefined && area.points !== undefined) { // both polygons
            ...
            return true;
        } else if (this.center !== undefined && area.points !== undefined) { // circle vs. polygon
            return circleIntersectsPolygon(this, area);
        } else if (this.points !== undefined && area.center !== undefined) { // polygon vs. circle
        ...
        function circleIntersectsPolygon(circle, polygon) {
            ...
            for (var i = 0; i < polygon.points.length; i++) {
                if (circle.coversPoint(polygon.points[i])) return true;
            }
            ...
            for (var i = 0, j = polygon.points.length - 1; i < polygon.points.length; j = i++) {
                ...
                var delta = circle.radius * circle.radius * dr * dr - D * D;
                if (delta >= 0) { // if the discriminant is nonnegative, then the line and circle intersect
                    return true;
                }
            }
    
            // check if the circle is entirely inside the polygon
            if (polygon.coversPoint(circle.center)) return true;
    ...
    this.parse = function (js_obj) {
        try {
            if (js_obj.hasOwnProperty('points')) { // polygon
                this.points = js_obj.points.map(p => {
                    var pos = new l_pos();
                    pos.parse(p);
                    return pos;
                });
            } else if (js_obj.hasOwnProperty('center') && js_obj.hasOwnProperty('radius')) { // circle
