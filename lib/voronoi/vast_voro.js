...
function VAST_Voronoi(bbox) {
    ...
    */
    ...
    this.collidesID_Polygon = function(id, polygonCorners) { // Checks overlap with a polygon using SAT
        index1= id2idx[id]
        if (!result || !result.cells || !result.cells[index1]) {
            console.error("Invalid index or result data not available");
            return false; // or handle the error appropriately
        }
        var poly1 = result.cells[index1].halfedges.map(he => ({x: he.edge.va.x, y: he.edge.va.y}));
        var poly2 = polygonCorners.map(pos => ({x: pos.x, y: pos.y}));
        ...
        for (var i = 0; i < polygons.length; i++) {
            ...
            for (var i1 = 0; i1 < polygon.length; i1++) {
                ...
                var p1 = polygon[i1];
                var p2 = polygon[i2];
                var normal = { x: p2.y - p1.y, y: p1.x - p2.x }; // Perpendicular to the edge
    
                // Project all points of both polygons onto the axis normal
                var minA, maxA, minB, maxB;
                minA = maxA = undefined;
                for (var j = 0; j < polygon.length; j++) {
                    var projected = normal.x * polygon[j].x + normal.y * polygon[j].y;
                ...
                for (var j = 0; j < polygonB.length; j++) {
                    var projected = normal.x * polygonB[j].x + normal.y * polygonB[j].y;
                    if (minB === undefined || projected < minB) minB = projected;
                    if (maxB === undefined || projected > maxB) maxB = projected;
                }
    
                // Check if there is a gap between the two projections
                if (maxA < minB || maxB < minA) {
                    return false; // No collision if there is a gap
                }
            }
        }
        return true; // Collision if no gap found
    }
