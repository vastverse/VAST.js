...
class MQTTSimulator {
    ...
    createClient(clientId, host, port, x, y, r) {
        return new Promise((resolve, reject) => {
            ...
            let connected = false;
            let timeoutHandle = null;  // ✅ Define here
    
            client.on('connect', () => {
                connected = true;
                if (timeoutHandle) clearTimeout(timeoutHandle);  // ✅ Now works
    ...
    async processLine(line, lineNumber) {
        ...
        switch (command) {
            ...
            case 'subscribe': {
                ...
                const topic = parts.slice(5).join(' ');  // ✅ Handle multi-word topics
            ...
            case 'publish': {
                ...
                const message = parts.slice(6).join(' ').replace(/^"|"$/g, '');  // ✅ FIXED
            ...
            case 'unsubscribe': {
                ...
                const unsubTopic = parts.slice(5).join(' ');  // ✅ Changed
