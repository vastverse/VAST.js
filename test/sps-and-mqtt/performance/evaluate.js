// performanceEvaluator.js - Multi-file version with conditional AoI matching
const fs = require('fs');
    ...
    return new Promise((resolve) => {
        ...
        rl.on('line', (line) => {
            try {
                ...
                // Skip non-JSON lines (like broker log messages)
            }
        });
        ...
        rl.on('close', () => {
            ...
            logToFile(`  Event type breakdown: ${JSON.stringify(eventTypeCounts)}`);
            logToFile(`    1=CLIENT_JOIN, 2=CLIENT_CONNECT, 3=CLIENT_LEAVE, 4=CLIENT_MIGRATE`);
            logToFile(`    6=SUB_NEW, 7=SUB_DELETE, 9=PUB, 10=RECEIVE_PUB`);
            ...
            for (var idx in sortedEvents) {
                ...
                // ✅ COLLECT SUB_NEW - Always collect AoI (needed for SPS/SPMQTT)
                if (event.event === 6) { // SUB_NEW
                    if (event.sub && event.sub.subID) {
                        var sub = event.sub;
                        var extractedChannel = extractChannelFromSPFormat(sub.channel);
                        
                        subscriptions[sub.subID] = {
                            subID: sub.subID,
                            clientID: sub.clientID,
                            channel: extractedChannel, // ✅ Extract channel name
                            aoi: event.sub.aoi,
                ...
                if (event.event === 9) { // PUB
                    if (event.pub && event.pub.pubID) {
                        var pubID = event.pub.pubID;
                        pubTimestamps[pubID] = event.time;
                        var extractedChannel = extractChannelFromSPFormat(event.pub.channel);
            ...
            logToFile(`  Collected: ${subCount} subscriptions, ${pubCount} publications, ${receipts.length} receipts`);

            // ✅ CONDITIONAL MATCHING: Use AoI for SPS/SPMQTT, channel-only for pure MQTT
            const isMQTTOnly = label.toUpperCase() === 'MQTT' || (label.toUpperCase().includes('MQTT') && !label.toUpperCase().includes('SP'));
            var matchCount = 0;
            ...
            for (var pubID in publications) {
                ...
                for (var subID in subscriptions) {
                    ...
                    // Channel check (required for both)
                    if (sub.channel !== pub.channel) {
                        channelMismatch++;
                        continue;
                    }
                    
                    // AoI check (only skip for pure MQTT)
                    if (!isMQTTOnly && pub.aoi && sub.aoi) {
                        if (!compareAoI(pub.aoi, sub.aoi)) {
                            aoiMismatch++;
                            continue;
                        }
                    }
            ...
            logToFile(`  System: ${isMQTTOnly ? 'MQTT (channel-only)' : 'SPS/SPMQTT (with AoI filtering)'}`);
            logToFile(`  Matching details: ${matchCount} total matches (channel mismatches: ${channelMismatch}, AoI mismatches: ${aoiMismatch})`);

            // Match receipts to publications
            var receiptsMatched = 0;
            var receiptsUnmatched = 0;
            ...
            // ✅ DEBUG: Log receipt matching
            logToFile(`  Receipt matching: ${receiptsMatched} matched, ${receiptsUnmatched} unmatched`);
            ...
            logToFile(`  Metrics: ${totalExpected} expected, ${totalReceived} received, ${undeliveredPubs.length} undelivered`);
            ...
            if (latencies.length > 0) {
                ...
                latencyStats = {
                    ...
                    median: calculateMedian(latencyValues).toFixed(2),
                    values: latencyValues
                };
                logToFile(`  Latency stats - Min: ${latencyStats.min}ms, Max: ${latencyStats.max}ms, Avg: ${latencyStats.avg}ms, Median: ${latencyStats.median}ms`);
            }
...
async function processAllFiles() {
    ...
    for (const [label, result] of Object.entries(allResults)) {
        ...
        logToFile(`  Total Expected: ${result.totalExpected}`);
        logToFile(`  Total Received: ${result.totalReceived}`);
        logToFile(`  Latency (avg): ${result.latencyStats.avg || 'N/A'}ms`);
        logToFile(`  Latency (min): ${result.latencyStats.min || 'N/A'}ms`);
        logToFile(`  Latency (max): ${result.latencyStats.max || 'N/A'}ms`);
        logToFile(`  Undelivered: ${result.undeliveredCount}`);
        logToFile(`  Duplicates: ${result.duplicateCount}`);
        logToFile('');
    }
        ...
        <div id="${sanitized}" class="tabcontent">
            ...
            <section class="metrics-grid">
                ...
                <div class="metric-card">
                    ...
                    <div class="metric-value">${latencyStats.median || 'N/A'}<span class="metric-unit">ms</span></div>
                ...
                <div class="metric-card">
                    <div class="metric-label">Expected Receipts</div>
                    <div class="metric-value">${result.totalExpected}</div>
                    <div class="metric-description">${result.totalReceived} received</div>
                </div>
            </section>
...
<body>
    <div class="container">
        <header>
            ...
            <p>Geo-spatial & Channel-Based Pub/Sub Analysis</p>
...
function generateComparisonTab(allResults) {
    ...
    const avgLatencies = labels.map(l => parseFloat(allResults[l].latencyStats.avg || 0));
...
// ✅ Helper function to extract channel name from sp:<...> format
function extractChannelFromSPFormat(channel) {
    if (!channel) return channel;
    
    // If it's in sp: <{"..."}> or sp:<{"..."}> format, try to extract the channel field
    // Handle both "sp: <" and "sp:<" formats
    if ((channel.startsWith('sp:') || channel.startsWith('sp :'))) {
        try {
            // Find the JSON part between < and >
            const startIdx = channel.indexOf('<');
            const endIdx = channel.lastIndexOf('>');
            
            if (startIdx !== -1 && endIdx !== -1) {
                const jsonStr = channel.substring(startIdx + 1, endIdx);
                const obj = JSON.parse(jsonStr);
                return obj.channel || channel;
            }
        } catch (e) {
            console.error('Error parsing channel format:', channel, e.message);
            return channel;
        }
    }
    return channel;
